...
class LightCurve(object):
    def __init__(self, zylclist, fnames=None, names=None, set_subtractmean=True, qlist=None):
        ...
        fnames: list of strings, optional
            Source file names of each individual light curve, plus the file
            route as the initial element (default: None).
        ...
        """
        # check if filename list is string or list of strings
        ...
        if names is None :
            ...
            if len(names) != self.nlc :
                ...
                self.names = names
        if fnames is None :
            # put the lack of a filename is if that's the case
            self.fnames = ['No file name give' for i in xrange(self.nlc)]
        else :
            if len(fnames) != self.nlc + 1:
                raise RuntimeError("names should match the dimension of zylclist + 1")
            else :
                self.fnames = fnames
    ...
    def plot(lc, set_pred=False, obs=None, marker="o", ms=4, ls='None', \
                    lw=1, figout=None, figext=None) :
        """ Plot light curves.
            ...
            if set_pred :
                ...
                if obs is not None :
                    ax.errorbar(obs.jlist[i], obs.mlist[i]+obs.blist[i], 
                            yerr=obs.elist[i], 
                            ecolor='k', marker=marker, ms=ms, mfc=mfc, 
                            mec='k', ls=ls, lw=lw,
                ...
                if np.sum(lc.elist[i]) == 0.0 :
                    ...
                    ax.errorbar(lc.jlist[i], lc.mlist[i]+lc.blist[i], 
                        yerr=lc.elist[i], ecolor='k', marker=marker, ms=ms, 
                        mfc=mfc, mec='k', ls=ls, lw=lw,
                        label=lc.names[i])
    ...
    def save_lcarr(self, fname, set_overwrite=True, set_addmean=True,\
                     set_saveid=False) :
        ...
        set_addmean: bool, optional
            True to save original light curve values without no mean 
...
def get_data(lcfile, names=None, set_subtractmean=True, timeoffset=0.0):
    ...
    if isinstance(lcfile, basestring):
        ...
        try :
            ...
            raise RuntimeError("ERROR: input is neither a list/tuple nor a string")
        lclist = []
        fnames = [cwd]
        for i in range(len(lcfile)) :
            if os.path.isfile(lcfile[i]):
                lc = readlc_3c(lcfile[i])
                lclist.append(lc[0])
    ...
    if len(lclist) > 0:
        ...
        zydata = LightCurve(lclist, names=names,fnames=fnames,
                             set_subtractmean=set_subtractmean)
        return(zydata)
      
    else:
        print "No light curve object produced." 

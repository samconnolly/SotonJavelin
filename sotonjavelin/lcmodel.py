...
class Rmap_Model(object) :
    ...
    def do_mcmc(self, conthpd=None, lagtobaseline=0.3,            
                 laglimit="baseline", nwalkers=100, nburn=100, nchain=100,
                  threads=1, fburn=None, fchain=None, flogp=None, 
                   set_threading=False, blocksize=10000, set_verbose=True,
                    max_attempts=5):

        success = False
        attempts = 1

        while success == False and attempts < max_attempts + 1:
            try:
                self.run_mcmc(conthpdR=conthpd, lagtobaselineR=lagtobaseline,
                         laglimitR=laglimit, nwalkersR=nwalkers, nburnR=nburn,
                          nchainR=nchain, threadsR=threads, fburnR=fburn, 
                           fchainR=fchain, flogpR=flogp, 
                            set_threadingR=set_threading, blocksizeR=blocksize,
                             set_verboseR=set_verbose)                
                success = True
                ...
                if attempts <= max_attempts:
                    print("=========== WARNING! MCMC FAILED! ===========")
                    print("   Restarting, attempt #{0}".format(attempts))
                    print("=============================================")

        if success == True:
            print("MCMC successful")
        else:
            raise ValueError("Max attempts exceed, MCMC failed")

    def run_mcmc(self, conthpdR=None, lagtobaselineR=0.3, laglimitR="baseline", nwalkersR=100, nburnR=100, nchainR=100, threadsR=1, fburnR=None, fchainR=None, flogpR=None, set_threadingR=False, blocksizeR=10000, set_verboseR=True):
        """ Run MCMC sampling over the parameter space.
        ...
        laglimit: str or list of tuples.
            ...
            'baseline' means the boundaries are naturally determined by the
            duration of the light curves, or you can set them as a list with
            `nline` of tuples, with each tuple containing the (min, max) pair for
            each single line. 
        nwalker : integer, optional
            Number of walkers for `emcee` (default: 100).
        nburn : integer, optional
        ...
        set_verbose: bool, optional
            Turn on/off verbose mode (default: True).
        """
        # fill in running params
        conthpd=conthpdR
        lagtobaseline=lagtobaselineR
        laglimit=laglimitR
        nwalkers=nwalkersR
        nburn=nburnR
        nchain=nchainR
        ...
        fchain=fchainR
        flogp=flogpR
        set_threading=set_threadingR
        blocksize=blocksizeR
        set_verbose=set_verboseR
...
#            p0[:, 2+i*3] = p0[:,2+i*3]*self.rj*lagtobaseline
        ...
        try:
            pos, prob, state = sampler.run_mcmc(p0, nburn)
        # catch dodgy initial values and restart
        except ValueError:
        ...
        if fchain is not None :
            if set_verbose :
                print("save MCMC chains to %s"%fchain)
            chainfile = open(fchain,'w')
            for f in self.fnames:
                chainfile.write(str(f))
                chainfile.write('\t')
            chainfile.write('\n')
            np.savetxt(chainfile, sampler.flatchain)
            chainfile.close()
        if flogp is not None :
            if set_verbose :
                print("save logp of MCMC chains to %s"%flogp)
    ...
    def show_hist(self,lag_lims = [None,None], bins=100, lagbinsize=1.0, \
        ...
        if type(self) != Rmap_Model:
           
            print "ERROR: Rmap_model or list of data must be provided"
            print "Input is of type",type(self)
            return(1)
            ...
            for k in xrange(nlc-1):
                for i in xrange(2+k*3, 5+k*3) :
                    ...
                    if i == 2 : 
                        ...
                        ax.hist(flatchain[:,i], bins,normed=norm)
                        ax.set_xlabel(texs[i])
                        ax.set_ylabel("N")
    ...
    def save_hist_data(self,filename='javelin_hist.dat',lag_lims = [None,None],\
                     lagbinsize=1.0, bins=100, plot=False, prob_density=False):

        '''
        Save the necessary data for plotting Javelin histograms as a txt file
            
        inputs:
            model - the (Rmap) model to take data from
                filename (optional, string) - the name of the saved txt file
            
             lag_lims: numerical list (len 2), optional
                    Limits on lag histogram plotting
        				
             bins: integer, optional
                    number of bins in plots except time lag

             lagbinsize: float, optional
                    bin width for 'lag' (default:100).
                    
             plot: bool, optional
                     plot the histogram you're saving

             prob_density: bool, optional
                     plot the y-axis as probability density instead of no. models
            
        '''
        # get all necessary data, save to file!
        ...
        hist = model.flatchain.T

        low = []
        
        # set lag lims if chosen
              
        for i in range(len(hist)):
            low.append(min(hist[i]))    
        
        high = []
        
        for i in range(len(hist)):
            high.append(max(hist[i])) 
        ...
        # find bin sizes  
        binwidths = (high-low)/bins
        binwidths[2] = lagbinsize    

        nbin = []
        wbins = []

        # calculate bins for each data point
        for i in range(len(hist)):
            
            nbin.append(((hist[i]-low[i])/binwidths[i]).astype(int))

        bincounts = []

        # count of number of points in each bin, create array of bin edges
        for i in range(len(hist)):

            bincounts.append(np.bincount(nbin[i]))
            wbins.append(np.arange(low[i],high[i]+binwidths[i], binwidths[i]))

            # check the number of edges matches the number of bins
            diff = len(wbins[-1])-len(bincounts[-1])

            if diff > 0:
                for i in range(diff):
                    bincounts[-1] = np.append(bincounts[-1],0)
      
        # Convert no. models to probability density if desired
        if prob_density:

            bincounts  = np.array(bincounts)/float(len(hist[0]))
        ...
        for i in range(len(bincounts)):
            if len(bincounts[i]) > nmax:
                nmax = len(bincounts[i])
        
        for l in range(nmax):
            for c in range(len(bincounts)):
                #print len(bincounts[c])
                if l < len(bincounts[c]):
                    f.write(str(wbins[c][l]))
                    f.write('\t')
                    
                    f.write(str(bincounts[c][l]))
                    f.write('\t')
        ...
        print "saved histogram data as {0} in {1}".format(filename,cwd)
        ...
        if plot:
            ...
            for i in range(len(hist)):
                ...
                plt.bar(wbins[i],bincounts[i],width=binwidths[i])
                
                ax.set_xlabel(model.texs[i])
                if prob_density:
    ...
    def save_mod_data(self,filename='javelin_mod.dat'):

        '''
        Save model data relaing to each parameter as a txt file
        ...
        print "saved model data as {0} in {1}".format(filename,cwd)
    ...
    def load_chain(self, fchain, set_verbose=True):
        ...
        for i in range(len(fnames)):
            ...
            if fnames[i] != self.fnames[i]:
                if i==0:
                    print "WARNING: The files used to create this chain were not in this location at the time of creation:"
                else:
                    print "WARNING: The names of the files used to create this chain do not match those in this model:"
                print "Original:", fnames[i]
                print "Current:", self.fnames[i]
        print "------------------------------------------------------------"
        self.flatchain = np.array(flatchain).astype(float)
        self.flatchain_whole = np.copy(self.flatchain)
